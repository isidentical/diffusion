...
class Encoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_fourier_feature_transform: bool = False):
        super().__init__()
        ...
        self.use_fourier_feature_transform = use_fourier_feature_transform

        if use_fourier_feature_transform:
            self.pretransform = FourierFeatures(input_channels, n_min=7, n_max=8)
            self.conv_in = nn.Conv2d(self.pretransform.out_features, hidden_channels, kernel_size=3, padding=1)
            nn.init.kaiming_normal_(self.conv_in.weight, nonlinearity='linear')
        else:
            # Inital conv layer to get to the hidden dimensionality
            self.pretransform = nn.Identity()
...
class AutoEncoder(nn.Module):
    ...
    def __init__(self,
        ...
        self.config['use_fourier_feature_transform'] = use_fourier_feature_transform
        ...
        self.encoder = Encoder(input_channels=self.config['input_channels'],
                               ...
                               use_fourier_feature_transform=self.config['use_fourier_feature_transform'])
